...
export class InterviewsService {
  ...
  async transition(id: string, to: ContentStatus, user: { id: string; role: UserRole }, comment?: string) {
    ...
    const kind = to === ContentStatus.InReview ? 'submitted' : to === ContentStatus.Published ? 'published' : to === ContentStatus.Rejected ? 'rejected' : 'approved';
    await this.notificationsQueue.add('notify', { kind, entityType: 'InterviewNotice', entityId: id, actorId: user.id });
    await this.cache.invalidate('interviews:');
    return updated;
  }
